...
const Alert1: React.FC<{ host: string; onEnable: () => void }> = ({
  ...
  <CustomAlert
    ...
    description={`有効にすると、今後 ${host}/api/v4 以下からグループやプロジェクトの一覧を取得するようになります。GitLab以外のサイトでは有効にしないでください。`}
    endContent={
...
const Alert3: React.FC<{
  ...
  <CustomAlert
    ...
    description="このページのURLに対応していないか権限がありません。プライベートな項目を表示するにはアクセストークンを設定してください。"
    endContent={
      <>
        <Button
          as={Link}
...
const Alert4: React.FC = () => (
  <CustomAlert color="primary" title="現在のURLのクエリ文字列を引き継ぎます" />
);
